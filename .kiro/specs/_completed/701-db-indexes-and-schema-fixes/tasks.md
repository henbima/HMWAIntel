...
**Commit:** `fix(db): add 6 missing blueprint indexes and move sync_requests to hmso`
...
- [x] Listener uses `hmso`-scoped client — verified in `listener/src/index.ts` line 92
- [x] No code changes needed — both already target `hmso` schema

---

## Completion Checklist
- [x] Verify via SQL: `SELECT indexname FROM pg_indexes WHERE schemaname = 'hmso'` — all 6 new indexes confirmed
- [x] Verify via SQL: `SELECT schemaname FROM pg_tables WHERE tablename = 'sync_requests'` — returns `hmso` only
